...
import type { AppConfig, EncryptionDirectoryKind, EncryptionResult, LogEntry, RuntimeStatus } from "../shared/contracts";
...
const pages: Array<{ id: Page; label: string }> = [{ id: "runtime", label: "概览" }, { id: "rules", label: "转发规则" }, { id: "addresses", label: "转发地址" }, { id: "local", label: "本地变量" }, { id: "values", label: "登录缓存" }, { id: "cache", label: "缓存" }, { id: "encryption", label: "加密" }, { id: "logs", label: "日志" }, { id: "settings", label: "设置" }];
...
function App() {
  ...
  const selectEncryptionDirectory = async (kind: EncryptionDirectoryKind): Promise<string | undefined> => { const result = await window.forwarder.selectEncryptionDirectory(kind); if (result.canceled) return undefined; if (!result.ok || result.path === undefined) { setError(result.error ?? "选择加密目录失败"); return undefined; } setError(""); return result.path; };
